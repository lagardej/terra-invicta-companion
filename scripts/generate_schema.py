...
The schema is written to:
    src/tic/savefile/process/schema/<version>/schema.json

If a schema already exists for that version, it is merged with the new one.
...
_SCHEMA_ROOT = Path(__file__).parent.parent / "build" / "schema"
